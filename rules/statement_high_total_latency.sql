...
-- @caveats: SUM_TIMER_WAIT 单位是皮秒。已过滤系统 schema 与 digest 为空的汇总行。P_S 只保留受 performance_schema_digests_size 限制的 top 语句，超出的会汇总到 digest='' 行——所以这里看到的是"P_S 认为的 top"，不是绝对 top。CPU 时间列需要 performance_schema 的 CPU 计时 consumer。
