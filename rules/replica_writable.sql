...
-- @caveats: 该规则只在"已经存在复制通道"时才触发，因此不适用于承担写流量的主库。有些架构刻意让从库可写（例如多主、双写、或把从库当只读业务库但接受不一致），这类情况下应把本规则加入 skip 列表而不是调参。读取 P_S 复制表需要 REPLICATION CLIENT 权限，缺权限时本规则会被跳过而不是误报干净。下界写作 5.7.8 而非 5.7：本规则读了 super_read_only，该变量是 5.7.8 才引入的，在 5.7.0~5.7.7 上会报 1193。如果你的实例早于 5.7.8（实际不会有），把 @requires 里的判断降级为只读 read_only 即可。
-- @ref: -
--
